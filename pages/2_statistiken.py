...
def make_line_chart(df: pd.DataFrame):
    ...
    chart_df["sma10"] = chart_df.groupby("player")["average"].transform(
...
st.markdown(
    '<div class="page-subtitle">KPI-Ansicht mit Checkout-Tracking, Head-to-Head pro Kalendertag und CSV-/Mail-Export.</div>',
...
filtered = df[
    (df["play_date"].dt.date >= start_date)
    & (df["play_date"].dt.date <= end_date)
].copy()
...
mail_lines = [
    ...
    f"Checkout Hanno: {kpi_h['checkout']:.1f}%" if kpi_h["checkout"] is not None else "Checkout Hanno: —",
    f"Checkout Dominik: {kpi_d['checkout']:.1f}%" if kpi_d["checkout"] is not None else "Checkout Dominik: —",
]
mail_body = quote("\n".join(mail_lines))
...
with ex1:
    st.download_button(
        "CSV herunterladen",
        data=df_to_csv_download(filtered),
        file_name="road_to_180_export.csv",
        mime="text/csv",
