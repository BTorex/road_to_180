...
from services import fetch_averages, player_image
...
def record_card(rank: int, row):
    ...
    comment_html = f"<div class='small-muted' style='margin-top:0.35rem;'>{html.escape(str(comment))}</div>" if comment else ""
    card = f"""
...
def player_summary(player: str, df, color: str):
    ...
    count = len(pdf)
    st.markdown('<div class="page-title">Rekorde</div>', unsafe_allow_html=True) if False else None
...
st.markdown('<div class="page-subtitle">Besser lesbare Rekorde mit Spielerbildern, sauberem HTML und zusätzlichen Leistungswerten.</div>', unsafe_allow_html=True)
