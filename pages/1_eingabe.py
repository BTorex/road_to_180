...
from services import PLAYERS, add_checkout_metrics, delete_average, fetch_averages, insert_average, safe_toast, update_average
...
st.markdown('<div class="page-subtitle">Mobile-first Eingabe mit optionalem Checkout-Tracking und Inline-Edit per Expander.</div>', unsafe_allow_html=True)
...
with st.container():
    ...
    with right:
        ...
        st.markdown('<div class="section-label" style="margin-top:0.4rem;">Checkout-Tracking</div>', unsafe_allow_html=True)
...
if history_df.empty:
    ...
    for _, row in history_df.iterrows():
        player = row["player"]
        label = f"ID {int(row['id'])} · {row['play_date'].strftime('%d.%m.%Y')} · {player} · {float(row['average']):.1f}"
        with st.expander(label, expanded=False):
            ...
            b1, b2 = st.columns(2)
            with b1:
                if st.button("💾 Speichern", use_container_width=True, key=f"save_{int(row['id'])}", disabled=new_hits > new_darts):
                    try:
                        update_average(
                            int(row["id"]),
                            new_avg,
