...
# UX Design: Dezenter Erklärtext
st.markdown("<div style='color: #888; font-size: 0.95rem; margin-bottom: 20px; text-align: center;'>Trage hier eure neuesten Lags ein oder korrigiere alte Daten.</div>", unsafe_allow_html=True)

main_tab1, main_tab2 = st.tabs(["📝 Neu eintragen", "⚙️ Historie bearbeiten"])

with main_tab1:
    with st.container(border=True):
        st.write("📅 **Spieldatum festlegen**")
        selected_date = st.date_input("Datum", format="DD.MM.YYYY", label_visibility="collapsed")
        ...
        with p_tab1:
            ...
            hanno_avg = st.number_input("3er Schnitt (Hanno)", min_value=0.0, max_value=180.0, step=0.1, key="h_avg_new")
            hanno_comment = st.text_input("Kommentar (optional)", placeholder="z.B. 170er Finish verpasst...", key="h_comm_new")
        ...
        with p_tab2:
            ...
            dominik_avg = st.number_input("3er Schnitt (Dominik)", min_value=0.0, max_value=180.0, step=0.1, key="d_avg_new")
            dominik_comment = st.text_input("Kommentar (optional)", placeholder="z.B. Neues Setup probiert...", key="d_comm_new")
...
with main_tab2:
    with st.container(border=True):
        st.write("### ⚙️ Eintrag anpassen")
        st.markdown("<div style='color: #888; font-size: 0.85rem; margin-bottom: 15px;'>Wähle ein vergangenes Spiel, um den Average zu korrigieren oder es zu löschen.</div>", unsafe_allow_html=True)
        ...
        if response.data:
            ...
            if selected_entry:
                ...
                new_comment = st.text_input("Kommentar aktualisieren", value=edit_comment if edit_comment else "")
                ...
                with col_b:
                    if st.button("🗑️ Löschen", use_container_width=True):
                        ...
                        st.error("Eintrag gelöscht!")

st.divider()

st.write("### 📊 Letzte Aktivitäten")
response_recent = supabase.table("dart_averages").select("*").order("id", desc=True).limit(5).execute()
if response_recent.data:
    ...
    if 'comment' not in df_recent.columns: df_recent['comment'] = ""
    df_recent['Kommentar'] = df_recent['comment'].fillna("")
    
    st.dataframe(df_recent.rename(columns={"player": "Wer", "average": "Schnitt"})[["Datum", "Wer", "Schnitt", "Kommentar"]], 
